...
//    warn!("exporting feeds {:?}", to_store);
    ...
    root.attributes.insert("version".to_owned(), "1.0".to_owned());
    ...
    for (folder, elements) in to_store {
        ...
        folder_element.attributes.insert("text".to_owned(), folder.clone());
        folder_element.attributes.insert("title".to_owned(), folder.clone());
        for feed in elements {
            let mut outline = Element::new("outline");
                outline.attributes.insert("type".to_owned(), "rss".to_owned());
                outline.attributes.insert("text".to_owned(), feed.url.clone());
                outline.attributes.insert("xmlUrl".to_owned(), feed.url.clone());
    ...
    fs::write(to_file, format!("{}", document)).expect(&format!("Unable to write file {:?}", to_file));
