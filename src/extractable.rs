...
pub trait Extractable<SourceFeed> : Dated {
    ...
    fn extract_content(&self, feed:&Feed, source_feed:&SourceFeed, settings:&Settings) -> String {
        TERA.render("message.html", &self.build_context(feed, source_feed, settings)).unwrap()
    }

    fn write_to_imap(&self, feed:&Feed, source:&SourceFeed, settings:&Settings, email:&mut Imap) {
    ...
    fn get_processed_content(&self, feed:&Feed, settings:&Settings)->String {
        ...
        if feed.config.inline_image_as_data || settings.config.inline_image_as_data {
            ...
            debug!("We should inline image as base64 data for {}", self.get_id(settings));
            document = image_to_data::transform(document, feed, settings);
        }
        let mut bytes = vec![];
        document.serialize(&mut bytes).expect(&format!("Unable to read entry {:?} of feed {}\nConsider sending a bug report !",
            self.get_links(settings), feed.url));
